<label class="flex items-center">
    <x-jet-checkbox id="HabFeF" name="HabFeF" value="hab" onchange="showContent()"/>
    <span class="ml-2 text-sm text-gray-600"> Habilitar Fecha de finalizacion </span>
</label>
  <div id="fechafinal" x-data="app()" x-init="[initDate(), getNoOfDays()]" x-cloak style="display: none;">
    <div class="container mx-auto py-2 md:py-10">
      <div class="mb-5 w-64">
        <label for="datepicker" class="font-bold mb-1 text-gray-700 block">Fecha de finalizacion</label>
        <div class="relative">

          <input type="hidden" name="ffinal" x-ref="date" :value="datepickerValue"/>
  ...
  <script type="text/javascript">
    ...
    function showContent() {
        var element = document.getElementById("fechafinal");
        var check = document.getElementById("HabFeF");
        if (check.checked) {
    ...
    function app() {
      ...
      if("<?php echo $employee->fecha_finalizacion ?>"){

       var fecha = "{{$employee->fecha_finalizacion}}".replace(/^(\d{4})-(\d{2})-(\d{2})$/g, '$2-$3-$1');
     }
